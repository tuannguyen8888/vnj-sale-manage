   ...
   <p><a href='{{g("return_url")}}'><i class='fa fa-chevron-circle-{{ trans('crudbooster.left') }}'></i> &nbsp; {{trans('crudbooster.form_back_to_list',['module'=>urldecode(g('label'))])}}</a></p>
    ...
    <div class="box">
      <div class="box-header">  
        ...
        @endif
        <div class="box-tools pull-{{ trans('crudbooster.right') }} {{$is_search_form?'customize-form-search':''}}" style="position: relative;margin-top: -5px;margin-right: -10px; {{$is_search_form?'width: 100%':''}}">
            @if($button_filter && !$is_search_form )
            ...
            <form method='get' style="display:inline-block;{{$is_search_form?'width: calc(100% - 60px);':'width: 260px;'}}" action='{{Request::url()}}'>
                ...
                <div class="input-group">
                  <input type="text" name="q" value="{{ Request::get('q') }}" class="form-control input-sm pull-{{ trans('crudbooster.right') }}" placeholder="{{trans('crudbooster.filter_search')}}"/>
                    ...
                    ?>
                  {!! CRUDBooster::getUrlParameters($exception_paras) !!}
                  <div class="input-group-btn">
                    @if(Request::get('q') || $has_search_value)
                    <?php
