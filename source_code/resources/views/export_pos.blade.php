	...
	<div>
		...
		<div class='panel panel-default'>
			<div class='panel-heading'>
				<strong><i class="fa fa-diamond"></i> {{$mode=='new'?'Tạo mới Đơn Hàng Nhanh':(mode=='edit'?'Sửa Đơn Hàng Nhanh':'Chi tiết Đơn Hàng Nhanh')}}</strong>
			</div>
						...
						<div class="row">
							...
							<div class="col-sm-10">
								<div class="input-group">
									...
									<input type="text" name="customer_phone" id="customer_phone" class="form-control" placeholder="Số ĐT" style="width: 10%">
									<input type="text" name="customer_address" id="customer_address" class="form-control" placeholder="Địa chỉ" style="width: 60%">
					...
					<div class="col-sm-12">
						...
						<div class="form-group header-group-1">
							<div class="row">
								...
								<div class="col-sm-2">
									<div class="input-group" >
										<input id="order_date" type="text" class="form-control" placeholder="Ngày đơn hàng" required>
								...
								<div class="col-sm-2">
									<input type="text" name="sampling_discount" id="sampling_discount" class="form-control money" placeholder="Chiết khấu chung">
								</div>
							...
							<div class="row">
								<label class="control-label col-sm-1">Tuổi vàng</label>
								<label class="control-label col-sm-1 text-right">Đồ đúc </label>
								<div class="col-sm-2">
									<input type="text" name="gold_age_1" id="gold_age_1" class="form-control money" value="{{CRUDBooster::getSetting('do_duc')}}" placeholder="Tuổi đồ đúc">
								</div>
								<label class="control-label col-sm-2 text-right">Đồ bộng </label>
								<div class="col-sm-2">
									<input type="text" name="gold_age_2" id="gold_age_2" class="form-control money" value="{{CRUDBooster::getSetting('do_bong')}}" placeholder="Tuổi đồ bộng">
								</div>
								<label class="control-label col-sm-2 text-right">Hàng khác</label>
								<div class="col-sm-2">
									<input type="text" name="gold_age_3" id="gold_age_3" class="form-control money" value="{{CRUDBooster::getSetting('tuoi_hang_khac')}}" placeholder="Tuổi hàng khác">
								</div>
							</div>
							<div class="row">
						...
						<div class="form-group header-group-1">
							<table id="table_order_details" class='table table-bordered'>
								<thead>
								<tr class="bg-success">
									<th>#</th>
									<th>STT</th>
						...
						<div class="form-group header-group-2">
							<table id="table_info" class='table table-bordered'>
								...
								<tr>
									<th class="no-padding">
										<div class="input-group" >
											<input id="pay_date" type="text" class="form-control" placeholder="Ngày thu" required>
											<div class="input-group-addon bg-gray">
												<i class="fa fa-calendar"></i>
											</div>
										</div>
	...
	<script type="application/javascript">
                            ...
                            success: function (data) {
                                if (data && data.product) {
                                    if (data.product.qty == 0) {
                                        ...
                                        switch (data.product.product_code.substr(0,1).toUpperCase()) {
                                            case 'V': //đồ đúc
                                        ...
                                        data.product.discount_machining_fee = $('#sampling_discount').val()?Number($('#sampling_discount').val().replace(/,/g, '')):null;
        ...
        function addNewSaleOrderDetail(dataRow) {
            ...
            let html = `<tr id="order_detail_` + dataRow.id + `">` +
                `<th class="text-center"><a style="cursor: pointer;" onclick="removeSaleOrderDetail(` + dataRow.id + `)"><i class="fa fa-remove text-red"></i></a></th>` +
                `<th class="text-right" id="no_` + dataRow.id + `">${dataRow.no}</th>` +
                `<th>${dataRow.bar_code}</th>` +
                `<th>${dataRow.product_type_id}</th>` +
                `<th>${dataRow.product_name}</th>` +
                `<th>${dataRow.product_code}</th>` +
                `<th class="text-right">${dataRow.age.toLocaleString('en-US')}</th>` +
                ...
                `<th class="text-right">${dataRow.exchange_g10.toLocaleString('en-US')}</th>` +
                ...
                `<th>${dataRow.product_group_id}</th>` +
                `<th>${dataRow.stock_id}</th>` +
                `</tr>`;
        ...
        function validate() {
            ...
            if(!valid) {
                ...
                if(order_details.length <= 100
                    && !((total_weight_calc - 0.003) <= actual_weight && actual_weight <= (total_weight_calc + 0.003)))
                {
                    valid = false;
                    $(`#actual_weight`).addClass('invalid');
                    swal("Thông báo", "Tổng cân thực tế chưa hợp lệ, vui lòng kiểm tra lại.", "warning");
                }
                if(order_details.length > 100
                    && !((total_weight_calc - 0.004) <= actual_weight && actual_weight <= (total_weight_calc + 0.004)))
                {
                    valid = false;
                    $(`#actual_weight`).addClass('invalid');
                    swal("Thông báo", "Tổng cân thực tế chưa hợp lệ, vui lòng kiểm tra lại.", "warning");
